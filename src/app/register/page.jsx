...
// Default customer role ID ΓÇö update if your backend uses a different ID
const CUSTOMER_ROLE_ID = 2;

const strengthConfig = {
  weak:   { color: '#EF4444', bar: 33,  label: 'Weak' },
  medium: { color: '#FF6B35', bar: 66,  label: 'Medium' },
...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    const toastId = toast.loading('Creating your accountΓÇª');
    try {
      ...
      toast.update(toastId, {
        render: '≡ƒÄë Account created! Please log in.',
        type: 'success',
        isLoading: false,
        autoClose: 3000,
      });
      ...
      <Box sx={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', bgcolor: '#F1F5F0', p: { xs: 3, sm: 4 }, overflow: 'auto', height: '100vh' }}>
        <Box sx={{ width: '100%', maxWidth: 400, py: 2 }}>
          ...
          <Box component="form" onSubmit={handleSubmit} noValidate sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            ...
            {/* Gender + DOB ΓÇö side by side */}
            <Box sx={{ display: 'flex', gap: 2 }}>
              {/* Gender */}
              <FormControl fullWidth>
                <InputLabel id="gender-label" sx={{ '&.Mui-focused': { color: '#16A34A' } }}>
                  Gender
                </InputLabel>
                <Select
