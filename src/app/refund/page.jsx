...
const steps = [
  ...
  { step: '03', title: 'Pickup Arranged', desc: 'We arrange a free pickup from your delivery address within 2ΓÇô3 business days of approval.' },
  { step: '04', title: 'Refund Processed', desc: 'Once we receive and inspect the product, your refund is processed within 5ΓÇô7 business days to your original payment method.' },
];
...
const sections = [
  ...
  },
  {
    ...
    content: `Refunds are credited to your original payment method. UPI and net banking refunds typically process within 3ΓÇô5 business days. Credit/debit card refunds may take 5ΓÇô7 business days depending on your bank. Cash on Delivery orders will receive a bank transfer refund.`,
  },
  {
    title: 'Cancellations',
    content: `Orders can be cancelled within 2 hours of placement for a full refund. Once an order is shipped, it cannot be cancelled. For prepaid orders cancelled after 2 hours, a full refund will be issued within 5ΓÇô7 business days.`,
  },
  {
  ...
  return (
    <MainLayout>
      <Box sx={{ background: 'linear-gradient(135deg, #16A34A 0%, #4ADE80 100%)', py: 6, color: '#FFFFFF', textAlign: 'center' }}>
        <Container maxWidth="md">
          ...
          <Typography sx={{ color: 'rgba(255,248,240,0.75)', fontSize: 16 }}>
            7-day easy returns ΓÇó Free pickups ΓÇó Fast refunds
          </Typography>
        </Container>
      </Box>
      ...
      <Container maxWidth="md" sx={{ py: 7 }}>
        {/* Process steps */}
        <Typography variant="h5" sx={{ fontWeight: 800, color: '#16A34A', mb: 3, textAlign: 'center' }}>
        ...
        >
          {steps.map((s) => (
            <Paper key={s.step} sx={{ p: 3, borderRadius: 3, textAlign: 'center', boxShadow: '0 4px 20px rgba(22,163,74,0.08)' }}>
              <Box
                sx={{
                  width: 52, height: 52, borderRadius: '50%', bgcolor: '#16A34A',
              ...
              <Typography variant="subtitle2" sx={{ fontWeight: 700, color: '#16A34A', mb: 1 }}>{s.title}</Typography>
              <Typography variant="caption" sx={{ color: '#4B5563', lineHeight: 1.7 }}>{s.desc}</Typography>
            </Paper>
          ))}
        </Box>
